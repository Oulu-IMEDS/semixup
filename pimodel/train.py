...
from pimodel.utils import parse_item, init_transforms, parse_target_accuracy_meter
from pimodel.utils import cond_accuracy_meter, parse_class
from pimodel.losses import PiModelLoss
...
if __name__ == "__main__":
    ...
    # model = DisCustomVGG2(nc=1, ndf=args.n_features, drop_rate=args.drop_rate).to(device)
    model = make_model(nc=1, model_name=args.model_name, ndf=args.n_features, drop_rate=args.drop_rate).to(device)
    crit = PiModelLoss(alpha=1.0).to(device)
    ...
    data_provider = pimodel_data_provider(root=args.root, model=model, train_labeled_data=train_labeled_data,
                                          train_unlabeled_data=train_all_data,
    ...
    # swa_start = int(len(data_provider.get_loader_by_name('labeled_train'))*0.75)
